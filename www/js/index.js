var app = {
  ...
  initApigee: function() {
      app.data.creds = new Apigee.Client({
        orgName:'APIGEE_ORG_NAME',
        appName:'APIGEE_APP_NAME'
      });
      app.data.creds.login('APIGEE_USERNAME', 'APIGEE_PASSWORD');
      app.data.places = new Apigee.Collection({ 'client': app.data.creds, 'type': 'places'});
  },

  loadPlaces: function() {
    $.get('' + app.data.creds.token, function(data){
  ...
  loadGoogleApi: function() {
    ...
    loadScript('https://maps.googleapis.com/maps/api/js?key=INSERT_GOOGLE_DEVELOPER_API_KEY&libraries=places&callback=app.getDistance');
